...
//      3. Because of 1, printfBufferLength *must* be a factor of
//      CUPRINTF_MAX_LEN
//
...
__device__ static char *getNextPrintfBufPtr() {
    ...
    // the entire available buffer into per-thread blocks. Inefficient, but what
    // can you do.
    int thread_count =
    ...
    // We *must* have a thread buffer length able to fit at least two printfs
    // (one header, one real)
    ...
    if (hdr.magic != CUPRINTF_SM10_MAGIC) {
        ...
        *(cuPrintfHeaderSM10 *)(void *)myPrintfBuffer =
        ...
        // For initial setup purposes, we might need to init thread0's header
        // too (so that cudaPrintfDisplay() below will work). This is only run
        // once.
...
__device__ static char *cuPrintfStrncpy(char *dest, const char *src, int n,
    ...
    // chunks that size, and CUPRINTF_MAX_LEN is aligned with
    // CUPRINTF_ALIGN_SIZE.
...
static int doPrintfDisplay(int headings, int clear, char *bufstart,
    ...
    while (bufptr != endptr) {
        ...
        // Adjust our start pointer to within the circular buffer and copy a
        // block.
        ...
        } else if (!outputPrintfData(
                       printfbuf_local + hdr->fmtoffset,
...
extern "C" cudaError_t cudaPrintfInit(size_t bufferLen) {
    ...
    bufferLen =
        (bufferLen < (size_t)CUPRINTF_MAX_LEN) ? CUPRINTF_MAX_LEN : bufferLen;
    ...
    cudaMemcpyToSymbol(printfBufferLength, &printfbuf_len,
                       sizeof(printfbuf_len));
...
extern "C" cudaError_t cudaPrintfDisplay(void *outputFP, bool showThreadID) {
    ...
    if (magic == CUPRINTF_SM10_MAGIC) {
        ...
        while (blockptr < (printfbuf_device + printfbuf_len)) {
            ...
            if (hdr.offset > 0) {
                // For synchronous printfs, we must print from endptr->bufend,
                // then from start->end
                if (sync_printfs) {
    ...
    // For SM_11 and up, everything is a single buffer and it's simple
        ...
        char *bufptr = ((printfbuf_start - printfbuf_device) % printfbuf_len) +
                       printfbuf_device;
        char *endptr = ((printfbuf_end - printfbuf_device) % printfbuf_len) +
                       printfbuf_device;

        // For synchronous (i.e. after-kernel-exit) printf display, we have to
        // handle circular buffer wrap carefully because we could miss those
        // past "end".
        if (sync_printfs) {
    ...
    // exit otherwise another kernel launch with a different grid size could
    // conflict.
    if (sync_printfs) {
        cudaMemset(printfbuf_device, 0, printfbuf_len);
    }
